...
( function ( global, factory ) {
...

  {
    ...
    generateComponentTrace = function ( vm ) {
      if ( vm._isVue && vm.$parent ) {
        ...
        return '\n\nfound in\n\n' + tree
          .map( function ( vm, i ) {
            return ( String( i === 0 ? '---> ' : repeat( ' ', 5 + i * 2 ) ) + ( Array.isArray( vm ) ?
  ...
  function getInvalidTypeMessage( name, value, expectedTypes ) {
    ...
    if ( expectedTypes.length === 1 &&
      ...
      message = message + ( ' with value ' + expectedValue )
    }
    message = message + ( ', got ' + receivedType + ' ' )
    // check if we need to specify received value
    if ( isExplicable( receivedType ) ) {
      message = message + ( 'with value ' + receivedValue + '.' )
    }
    return message
  }
  ...
  function updateChildComponent(
    ...
    const hasDynamicScopedSlot = Boolean( ( newScopedSlots && !newScopedSlots.$stable ) ||
    ...
    const needsForceUpdate = Boolean( renderChildren || // has new static slots
    ...
    function removeAndInvokeRemoveHook( vnode, rm ) {
      if ( isDef( rm ) || isDef( vnode.data ) ) {
        ...
        if ( isDef( rm ) ) {
          ...
          rm.listeners = rm.listeners + listeners
        } else {
          // directly removing
  ...
  function genData( el ) {
    let data = ''
    if ( el.staticClass ) {
      data = data + ( 'staticClass:' + ( el.staticClass ) + ',' )
    }
    if ( el.classBinding ) {
      data = data + ( 'class:' + ( el.classBinding ) + ',' )
    }
    return data
  }
  ...
  function genData$1( el ) {
    let data = ''
    if ( el.staticStyle ) {
      data = data + ( 'staticStyle:' + ( el.staticStyle ) + ',' )
    }
    if ( el.styleBinding ) {
      data = data + ( 'style:(' + ( el.styleBinding ) + '),' )
    }
    return data
  }
  ...
  function parseHTML( html, options ) {
    ...
    while ( html ) {
      ...
      if ( !lastTag || !isPlainTextElement( lastTag ) ) {
        ...
        index = index + ( html.length - rest$1.length )
        html = rest$1
    ...
    function handleStartTag( match ) {
      ...
      const unary = isUnaryTag$$1( tagName ) || Boolean( unarySlash )
  ...
  function genHandlers(
    ...
    for ( const name in events ) {
      ...
      if ( events[ name ] && events[ name ].dynamic ) {
        dynamicHandlers = dynamicHandlers + ( name + ',' + handlerCode + ',' )
      } else {
        staticHandlers = staticHandlers + ( '"' + name + '":' + handlerCode + ',' )
      }
    }
  ...
  function genHandler( handler ) {
    ...
    for ( const key in handler.modifiers ) {
      if ( modifierCode[ key ] ) {
        genModifierCode = genModifierCode + modifierCode[ key ]
        // left/right
        if ( keyCodes[ key ] ) {
          keys.push( key )
        }
        ...
        genModifierCode = genModifierCode + genGuard(
          [ 'ctrl', 'shift', 'alt', 'meta' ]
  ...
  function genData$2( el, state ) {
    ...
    if ( el.component ) {
      data = data + ( 'tag:"' + ( el.tag ) + '",' )
    }
    // module data generation functions
    for ( let i = 0; i < state.dataGenFns.length; i++ ) {
      data = data + state.dataGenFns[ i ]( el )
    }
    // attributes
    if ( el.attrs ) {
      data = data + ( 'attrs:' + ( genProps( el.attrs ) ) + ',' )
    }
    // DOM props
    if ( el.props ) {
      data = data + ( 'domProps:' + ( genProps( el.props ) ) + ',' )
    }
    // event handlers
    if ( el.events ) {
      data = data + ( ( genHandlers( el.events, false ) ) + ',' )
    }
    if ( el.nativeEvents ) {
      data = data + ( ( genHandlers( el.nativeEvents, true ) ) + ',' )
    }
    // slot target
    // only for non-scoped slots
    if ( el.slotTarget && !el.slotScope ) {
      data = data + ( 'slot:' + ( el.slotTarget ) + ',' )
    }
    // scoped slots
    if ( el.scopedSlots ) {
      data = data + ( ( genScopedSlots( el, el.scopedSlots, state ) ) + ',' )
    }
    // component v-model
    if ( el.model ) {
  ...
  function genDirectives( el, state ) {
    ...
    for ( i = 0, l = dirs.length; i < l; i++ ) {
      ...
      if ( needRuntime ) {
        hasRuntime = true
        res = res + ( '{name:"' + ( dir.name ) + '",rawName:"' + ( dir.rawName ) + '"' + ( dir.value ? ( ',value:(' + ( dir.value ) + '),expression:' + ( JSON.stringify( dir.value ) ) ) : '' ) + ( dir.arg ? ( ',arg:' + ( dir.isDynamicArg ? dir.arg : ( '"' + ( dir.arg ) + '"' ) ) ) : '' ) + ( dir.modifiers ? ( ',modifiers:' + ( JSON.stringify( dir.modifiers ) ) ) : '' ) + '},' )
      }
    }
    if ( hasRuntime ) {
    ...
    if ( children.length ) {
      ...
      if ( children.length === 1 &&
        ...
        const normalizationType = checkSkip ?
          ...
          ''
        return ( String( ( altGenElement || genElement )( el$1, state ) ) + normalizationType )
      }
  ...
  function genProps( props ) {
    ...
    for ( let i = 0; i < props.length; i++ ) {
      ...
      if ( prop.dynamic ) {
        dynamicProps = dynamicProps + ( ( prop.name ) + ',' + value + ',' )
      } else {
        staticProps = staticProps + ( '"' + ( prop.name ) + '":' + value + ',' )
      }
    }
  ...
  function generateCodeFrame(
    ...
    for ( let i = 0; i < lines.length; i++ ) {
      count = count + ( lines[ i ].length + 1 )
      if ( count >= start ) {
        for ( let j = i - range; j <= i + range || end > count; j++ ) {
          if ( j < 0 || j >= lines.length ) {
            continue
          }
          res.push( ( String( j + 1 ) + ( repeat$1( ' ', 3 - String( j + 1 ).length ) ) + '|  ' + ( lines[ j ] ) ) )
